...
function setupSalesLogin() {
    ...
    loginForm.addEventListener('submit', (event) => {
        ...
        if (username !== ADMIN_USERNAME || password !== SALES_ADMIN_PASSWORD) {
            if (loginError) {
                loginError.textContent = 'Access denied. Only luxery may view this page.';
...
function loadEditableLogs() {
    ...
    return entries.length ? entries.sort((a, b) => new Date(b.date) - new Date(a.date)) : null;
}
...
function renderSaleCards(logs) {
    ...
    list.innerHTML = logs.map((entry, index) => {
        ...
        const rawIndex = rawLogs.findIndex(item => item.date === entry.date && item.rawText === entry.rawText && item.salesperson === entry.salesperson);
        const editButton = isAdmin && rawIndex >= 0 ? `<button class="sale-card-edit" data-index="${rawIndex}" title="Edit this entry">✏️</button>` : '';
        return `
            <div class="sale-card">
                ${editButton}
...
function renderLeaderboard(logs) {
    ...
    if (!rows.length) {
        leaderboard.innerHTML = '<div class="sales-empty">No sales in the last 30 days.</div>';
        return;
    }
...
function renderSummary(logs) {
    ...
    const cutoff = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    const recentCount = logs.filter(entry => new Date(entry.date) >= cutoff).length;
    ...
    if (summaryRevenue) summaryRevenue.textContent = formatCurrency(Math.round(totalRevenue));
}
...
function renderEditableLogSection(logs) {
    ...
    const textValue = localStorage.getItem('salesLogText') || generateLogText(logs);
...
function initializeEditableLogControls() {
    ...
    if (resetButton) {
        resetButton.addEventListener('click', () => {
            ...
            if (status) {
                status.textContent = 'Sales log reset. Reload the page to restore generated log view.';
                status.style.color = '#d4af37';
            }
            const logs = loadLogs();
            allSalesLogs = logs;
